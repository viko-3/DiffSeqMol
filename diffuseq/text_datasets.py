...
def helper_tokenize(sentence_lst, vocab_dict, seq_len):
    ...
    def random_mask(group_lst):
        masked_sequences = []
        for sequence in group_lst['input_id_x']:
            for i, token in enumerate(sequence):
                if token in vocab_dict.tokenizer.all_special_ids:
                    continue
                ...
                prob = random.random()
                if prob < 0.8:
                    sequence[i] = vocab_dict.tokenizer.mask_token_id
                elif prob < 0.9:
                    token_num = len(vocab_dict.tokenizer.vocab.values())
                    special_num = len(vocab_dict.tokenizer.all_special_ids)
                    token = list(vocab_dict.tokenizer.vocab.values())[random.randint(special_num, token_num - 1)]
                    sequence[i] = token
            masked_sequences.append(sequence)
        group_lst['input_id_x'] = masked_sequences
        return group_lst
