...
def main():
    ...
    import numpy as np
    my_array = np.load('/home/s2136015/Code/cyy_utils/DiffuLanguage/vocab.npy')
    res = model.get_embeds(th.tensor(my_array).cuda())
    array = res.cpu().detach().numpy()

    # 保存为 .npy 文件
    np.save('/home/s2136015/Code/cyy_utils/DiffuLanguage/vocab_emb.npy', array)
    ...
    for cond in tqdm(all_test_data):
        ...
        for temp in [-1]:
        #for temp in [-1, -(len(samples) // 2), -len(samples) + 1]:
