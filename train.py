...
def main():
    ...
    ### cyy加的
    if args.use_reconstruct_pt and args.load_reconstruct_pt is not None:
        model.load_state_dict(torch.load(args.load_reconstruct_pt))
    ###
    # model.cuda() #  DEBUG **

    pytorch_total_params = sum(p.numel() for p in model.parameters())

    logger.log('### The parameter count is {}'.format(pytorch_total_params))
    schedule_sampler = create_named_schedule_sampler(args.schedule_sampler, diffusion)

    logger.log('### Saving the hyperparameters to {}/training_args.json'.format(args.checkpoint_path))
    with open('{}/training_args.json'.format(args.checkpoint_path), 'w') as f:
